...
public class InputController
{
	...
	public float GetAxisInput()
	{
		...
		{
			inputValue = Input.acceleration.x;
		}
		else
		{
			if ( "" != AxisName )
	...
	public float GetAxisInput_Y()
	{
		float inputValue = 0.0f;

		if(Application.platform==RuntimePlatform.Android)
		{
			inputValue = ((Input.acceleration.y+0.5f));
		}
		else
		{
			if ( "" != AxisName )
	...
	public float GetAxisInput_Z()
	{
		float inputValue = 0.0f;
		
		if(Application.platform==RuntimePlatform.Android)
		{
			inputValue = ((Input.acceleration.z));
		}
		else
		{
